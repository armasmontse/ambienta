...
use App\Http\Requests\Admin\Settings\UpdateFileRequest;
...
class ManageSettingsController extends Controller
{
    ...
    public function index()
    {
        $data = [
			...
			'setting_collections'	=> setting('collections.pdf')
		];
	...
	public function fileSetting(UpdateFileRequest $request)
	{
		$setting = Setting::getSetting('collections.pdf');
		...
		return Redirect::back()->with('status', trans('manage_settings.collections.pdf.title') . ': ' . trans('manage_settings.success.update'));
	}
}
